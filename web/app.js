...
const inviteFromPath = location.pathname.match(/^\/invite\/([A-Za-z0-9]{48})$/)?.[1];
if (inviteFromPath) { localStorage.setItem('lumichat_pending_invite', inviteFromPath); history.replaceState({},'', '/'); }
const state = { token: localStorage.getItem('lumichat_token'), user: null, channels: [], users: [], requests: {incoming:[],outgoing:[]}, context: null, messages: [], upload: null, reply: null, ws: null, unread: JSON.parse(localStorage.getItem('lumichat_unread') || '{}'), call: null, audit: null };
const emojis = ['😀','😂','🥰','😍','🤔','😎','😭','😡','👍','👏','🙏','💪','🎉','❤️','🔥','✨','✅','👀'];
...
async function boot() {
  ...
  try {
    ...
    await Promise.all([loadChannels(), loadUsers(), loadFriendRequests()]);
    ...
    connectSocket();
    if (localStorage.getItem('lumichat_pending_invite')) setTimeout(handlePendingInvite, 250);
...
async function loadUsers() {
  const activeDmId = state.context?.type === 'dm' ? state.context.id : null;
  state.users = await api('/friends'); const users = state.users.filter(u => u.id !== state.user.id && u.active);
  $('#user-list').innerHTML = users.map(u => navItem('dm', u, `<span class="mini-avatar">${escapeHtml(initials(u.display_name))}</span>`, u.display_name)).join('') || '<p style="padding:8px 10px;color:var(--muted);font-size:11px">还没有联系人，点击＋精确添加</p>'; bindNavigation();
  if (activeDmId && !users.some(user => user.id === activeDmId) && state.channels[0]) {
    toast('该联系人已不在好友列表，历史记录仍然保留');
    await openContext({type:'channel', ...state.channels[0]});
  }
}
async function loadFriendRequests() { if (!state.user) return; state.requests = await api('/friend-requests'); const count=state.requests.incoming.length; $('#friend-requests').textContent=count ? `申请 ${count}` : '申请'; $('#friend-requests').classList.toggle('has-pending',count>0); }
...
async function openContext(context) {
  ...
  $('#friend-actions').hidden = context.type !== 'dm';
...
function requestRows(items, incoming) { return items.map(item => `<div class="friend-request-row"><div class="mini-avatar">${escapeHtml(initials(item.user.display_name))}</div><div><strong>${escapeHtml(item.user.display_name)}</strong><small>@${escapeHtml(item.user.username)} · UID ${item.user.id}</small></div><div>${incoming ? `<button data-accept-request="${item.id}" class="primary-mini" type="button">接受</button><button data-reject-request="${item.id}" type="button">拒绝</button>` : `<button data-cancel-request="${item.id}" type="button">取消</button>`}</div></div>`).join('') || `<p class="friend-empty">${incoming ? '暂时没有收到申请' : '暂时没有发出的申请'}</p>`; }
async function showFriendCenter() {
  try {
    const [, invite] = await Promise.all([loadFriendRequests(), api('/friend-invite')]);
    const qr = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(invite.qr_svg)}`;
    showDialog('好友与邀请', `<section class="friend-add"><h3>精确添加</h3><p>输入完整用户名或 UID。不会显示相似用户。</p><div class="friend-lookup"><input id="friend-query" maxlength="24" placeholder="完整用户名或 UID"><button id="lookup-friend" type="button" class="primary-mini">查找</button></div><div id="friend-result"></div></section><section class="friend-requests"><h3>好友申请</h3><div class="request-columns"><div><h4>收到的申请</h4>${requestRows(state.requests.incoming,true)}</div><div><h4>发出的申请</h4>${requestRows(state.requests.outgoing,false)}</div></div></section><section class="friend-invite"><div><h3>个人邀请</h3><p>UID ${state.user.id} · 链接和二维码可随时重新生成。</p><div class="invite-link"><input id="invite-link" readonly value="${escapeHtml(invite.url)}"><button id="copy-invite" type="button">复制</button></div><button id="regenerate-invite" class="text-danger" type="button">使旧链接失效并重新生成</button></div><img src="${qr}" alt="个人好友邀请二维码"></section>`, 'FRIENDS', () => bindFriendCenter(invite));
  } catch(e) { toast(e.message); }
}
function bindFriendCenter(invite) {
  $('#lookup-friend').onclick = async () => { const query=$('#friend-query').value.trim(); if (!query) return; $('#friend-result').innerHTML='<p class="friend-empty">正在精确查找…</p>'; try { const result=await api('/friends/lookup',{method:'POST',body:JSON.stringify({query})}); const u=result.user; const action=result.relationship==='none' ? '<button id="send-found-request" class="primary-mini" type="button">发送申请</button>' : `<span class="relationship-state">${result.relationship==='friend'?'已经是好友':result.relationship==='incoming'?'对方已申请你':'申请已发送'}</span>`; $('#friend-result').innerHTML=`<div class="friend-found"><div class="mini-avatar">${escapeHtml(initials(u.display_name))}</div><div><strong>${escapeHtml(u.display_name)}</strong><small>@${escapeHtml(u.username)} · UID ${u.id}</small></div>${action}</div>`; if ($('#send-found-request')) $('#send-found-request').onclick=async()=>{ try { await api('/friend-requests',{method:'POST',body:JSON.stringify({identifier:query})}); toast('好友申请已发送'); await showFriendCenter(); } catch(e) { toast(e.message); } }; } catch(e) { $('#friend-result').innerHTML=`<p class="friend-empty">${escapeHtml(e.message)}</p>`; } };
  $('#friend-query').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();$('#lookup-friend').click();}};
  document.querySelectorAll('[data-accept-request]').forEach(b=>b.onclick=()=>actOnRequest(b.dataset.acceptRequest,'accept'));
  document.querySelectorAll('[data-reject-request]').forEach(b=>b.onclick=()=>actOnRequest(b.dataset.rejectRequest,'reject'));
  document.querySelectorAll('[data-cancel-request]').forEach(b=>b.onclick=()=>actOnRequest(b.dataset.cancelRequest,'cancel'));
  $('#copy-invite').onclick=async()=>{await navigator.clipboard.writeText(invite.url);toast('邀请链接已复制');};
  $('#regenerate-invite').onclick=async()=>{if(!confirm('旧邀请链接和二维码会立即失效，确定重新生成吗？'))return;try{await api('/friend-invite/regenerate',{method:'POST'});await showFriendCenter();toast('已生成新的邀请链接');}catch(e){toast(e.message);}};
}
async function actOnRequest(id,action){try{await api(`/friend-requests/${id}/${action}`,{method:'POST'});await Promise.all([loadUsers(),loadFriendRequests()]);await showFriendCenter();toast(action==='accept'?'已成为好友':action==='reject'?'已拒绝申请':'已取消申请');}catch(e){toast(e.message);}}
async function handlePendingInvite(){const token=localStorage.getItem('lumichat_pending_invite');if(!token||!state.user)return;try{const result=await api('/friends/lookup',{method:'POST',body:JSON.stringify({invite_token:token})});const u=result.user;const stateText=result.relationship==='friend'?'你们已经是好友。':result.relationship==='incoming'?'对方已经向你发送申请，请在好友申请中处理。':result.relationship==='outgoing'?'你的好友申请正在等待对方确认。':'确认向这个用户发送好友申请。';const action=result.relationship==='none'?'<button id="accept-invite-link" type="button" class="primary">发送好友申请</button>':result.relationship==='friend'?'<button id="open-existing-friend" type="button" class="primary">打开联系人</button>':result.relationship==='incoming'?'<button id="open-request-center" type="button" class="primary">查看好友申请</button>':'<button id="finish-invite" type="button" class="primary">知道了</button>';showDialog('好友邀请',`<div class="invite-person"><div class="mini-avatar">${escapeHtml(initials(u.display_name))}</div><div><strong>${escapeHtml(u.display_name)}</strong><small>@${escapeHtml(u.username)}</small></div></div><p class="friend-empty">${stateText}</p><div class="dialog-actions"><button id="ignore-invite" type="button">暂不处理</button>${action}</div>`,'INVITE',()=>{const finish=()=>{localStorage.removeItem('lumichat_pending_invite');$('#dialog').close();};$('#ignore-invite').onclick=finish;if($('#accept-invite-link'))$('#accept-invite-link').onclick=async()=>{try{await api('/friend-requests',{method:'POST',body:JSON.stringify({invite_token:token})});finish();await loadFriendRequests();toast('好友申请已发送');}catch(e){toast(e.message);}};if($('#open-existing-friend'))$('#open-existing-friend').onclick=()=>{finish();const friend=state.users.find(x=>x.id===u.id);if(friend)openContext({type:'dm',...friend});};if($('#open-request-center'))$('#open-request-center').onclick=()=>{localStorage.removeItem('lumichat_pending_invite');showFriendCenter();};if($('#finish-invite'))$('#finish-invite').onclick=finish;});}catch(e){localStorage.removeItem('lumichat_pending_invite');toast(e.message);}}
async function showFriendActions(){const friend=state.context;if(!friend||friend.type!=='dm')return;showDialog('好友设置',`<div class="invite-person"><div class="mini-avatar">${escapeHtml(initials(friend.display_name))}</div><div><strong>${escapeHtml(friend.display_name)}</strong><small>@${escapeHtml(friend.username)} · UID ${friend.id}</small></div></div><p class="friend-empty">删除好友后历史记录仍会保留，但双方不能继续私聊或通话。</p><div class="dialog-actions"><button id="block-friend" class="text-danger" type="button">拉黑</button><button id="remove-friend" class="text-danger" type="button">删除好友</button></div>`,'CONTACT',()=>{$('#remove-friend').onclick=()=>removeOrBlockFriend(friend,false);$('#block-friend').onclick=()=>removeOrBlockFriend(friend,true);});}
async function removeOrBlockFriend(friend,block){if(!confirm(block?'拉黑后对方不能申请、私聊或呼叫你，确定继续吗？':'确定删除好友吗？历史聊天仍会保留。'))return;try{if(state.call)endCall(true);await api(`/friends/${friend.id}`,{method:block?'POST':'DELETE'});$('#dialog').close();state.context=null;await loadUsers();if(state.channels[0])await openContext({type:'channel',...state.channels[0]});toast(block?'已拉黑该用户':'已删除好友');}catch(e){toast(e.message);}}
$('#add-friend').onclick=showFriendCenter;$('#friend-requests').onclick=showFriendCenter;$('#friend-actions').onclick=showFriendActions;

$('#manage-button').onclick = () => showUserManager();
async function showUserManager() { const users=await api('/users'); const rows = users.map(u => `<div class="dialog-row"><div><p>${escapeHtml(u.display_name)} ${u.id === state.user.id ? '（你）' : ''}</p><small>@${escapeHtml(u.username)} · ${u.role === 'admin' ? '管理员' : '成员'}</small></div>${u.id === state.user.id ? '' : `<button class="quiet-button" data-manage="${u.id}">${u.active ? '停用' : '启用'}</button>`}</div>`).join(''); showDialog('用户管理', rows, 'ADMIN', () => document.querySelectorAll('[data-manage]').forEach(button => button.onclick = async () => { const u = users.find(item => item.id == button.dataset.manage); try { await api(`/users/${u.id}`, { method:'PATCH', body:JSON.stringify({role:u.role,active:!u.active}) }); await showUserManager(); } catch(e) { toast(e.message); } })); }
...
function showProfile() { showDialog('我的账号', `<div class="dialog-row"><div><p>${escapeHtml(state.user.display_name)}</p><small>@${escapeHtml(state.user.username)} · UID ${state.user.id} · ${state.user.role === 'admin' ? '管理员' : '成员'}</small></div></div><label>显示名称<input id="profile-name" maxlength="40" value="${escapeHtml(state.user.display_name)}"></label><div class="dialog-actions"><button id="open-friends" type="button">好友与邀请</button>${state.user.role === 'admin' ? '<button id="open-audit" type="button">全部记录</button><button id="open-admin" type="button">用户管理</button>' : ''}<button id="save-profile" type="button" class="primary">保存资料</button></div>`, 'PROFILE', () => { $('#save-profile').onclick = async () => { try { state.user = await api('/me',{method:'PATCH',body:JSON.stringify({display_name:$('#profile-name').value})}); updateProfileUi(); $('#dialog').close(); await loadUsers(); } catch(e) { toast(e.message); } }; $('#open-friends').onclick=showFriendCenter; if ($('#open-admin')) $('#open-admin').onclick = showUserManager; if ($('#open-audit')) $('#open-audit').onclick = showAudit; }); }
function showDialog(title, body, eyebrow, ready) { const dialog=$('#dialog'); $('#dialog-title').textContent = title; $('#dialog-eyebrow').textContent = eyebrow; $('#dialog-body').innerHTML = body; if (!dialog.open) dialog.showModal(); ready?.(); }
